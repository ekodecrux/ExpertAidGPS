...
// src/lib/emulatedMySQL.ts
var import_fs = __toESM(require("fs"), 1);
var import_path = __toESM(require("path"), 1);
var FILE_PATH = import_path.default.join(process.cwd(), "database_records_emulated.json");
function getDBTableName(rawName) {
...
function writeDB(state) {
  try {
    import_fs.default.writeFileSync(FILE_PATH, JSON.stringify(state, null, 2), "utf8");
  } catch (err) {
...
function evaluateCondition(row, conditionStr) {
  ...
  if (/is\s+null/i.test(cond)) {
    ...
    if (fieldMatch) {
      ...
      return row[field] === null || row[field] === void 0 || row[field] === "";
    }
  }
  ...
  if (opMatch) {
    ...
    if (expectedValStr.startsWith("'") && expectedValStr.endsWith("'") || expectedValStr.startsWith('"') && expectedValStr.endsWith('"')) {
      expectedValStr = expectedValStr.slice(1, -1);
    }
    const actualValue = row[field];
    const actualStr = actualValue === null || actualValue === void 0 ? "" : String(actualValue);
    if (operator === "=") {
...
function rowMatchesWhere(row, whereClause) {
  ...
  if (/\s+or\s+/i.test(whereClause)) {
    const parts2 = whereClause.split(/\s+or\s+/i);
    return parts2.some((p) => evaluateCondition(row, p));
  }
  const parts = whereClause.split(/\s+and\s+/i);
  return parts.every((p) => evaluateCondition(row, p));
}
var EmulatedMySQLConnection = class {
  async query(statement, params = []) {
    ...
    if (lowerSql.startsWith("select")) {
      ...
      if (selectMatch) {
        ...
        const filtered = tableRows.filter((r) => rowMatchesWhere(r, whereClause));
    ...
    if (lowerSql.startsWith("insert")) {
      ...
      if (insertMatch) {
        ...
        fields.forEach((f, idx) => {
          newRecord[f] = params[idx] !== void 0 ? params[idx] : null;
        });
        ...
        db[tableName] = db[tableName].filter((r) => r[idCol] !== pkValue);
    ...
    if (lowerSql.startsWith("delete")) {
      ...
      if (deleteMatch) {
        ...
        if (db[tableName]) {
          ...
          db[tableName] = db[tableName].filter((r) => !rowMatchesWhere(r, whereClause));
...
function getEmulatedMySQLConnection() {
...
async function sendCredentialsEmail(email, name, password) {
  ...
  const transporter = import_nodemailer.default.createTransport({
    ...
    greetingTimeout: 5e3,
    // 5 seconds greeting timeout
    socketTimeout: 5e3
    // 5 seconds socket timeout
  });
  try {
      ...
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
          ...
          <div style="background-color: #f1f5f9; padding: 20px; text-align: center; color: #94a3b8; font-size: 12px;">
            &copy; ${(/* @__PURE__ */ new Date()).getFullYear()} Expert GPS Solution. All rights reserved.
          </div>
        </div>
      `
    });
...
async function start() {
  ...
  try {
    const serviceAccountPath = import_path2.default.join(process.cwd(), "firebase-service-account.json");
    let credentialOption = void 0;
    let fallbackProjId = firebaseConfig.projectId;
    if (import_fs2.default.existsSync(serviceAccountPath)) {
      console.log("[Firebase Admin] Loading provided service account from firebase-service-account.json");
      const saData = JSON.parse(import_fs2.default.readFileSync(serviceAccountPath, "utf8"));
      credentialOption = (0, import_app.cert)(saData);
    ...
    if ((0, import_app.getApps)().length === 0) {
      if (credentialOption) {
        ...
        (0, import_app.initializeApp)({ projectId: fallbackProjId });
      }
      try {
        const authClient = new import_google_auth_library.GoogleAuth();
        const credentials = await authClient.getCredentials();
        console.log("Running with server identity:", credentials.client_email || "Default ADC");
      } catch (debugErr) {
      }
    }
  } catch (e) {
  ...
  try {
    const customDbId = firebaseConfig.firestoreDatabaseId && firebaseConfig.firestoreDatabaseId !== "remixed-firestore-database-id" && firebaseConfig.firestoreDatabaseId !== "(default)" ? firebaseConfig.firestoreDatabaseId : void 0;
    firestoreDb = customDbId ? (0, import_firestore.getFirestore)(void 0, customDbId) : (0, import_firestore.getFirestore)();
    try {
      ...
      (async () => {
        try {
          ...
          if (userRecord) {
            ...
            let connReplicate = null;
            try {
              connReplicate = await getMySQLConnection();
              const uDate = (/* @__PURE__ */ new Date()).toISOString();
  ...
  const getMySQLConfig = async () => {
    ...
    let dbProvider = "hostinger_vps";
    try {
      const configPath = import_path2.default.join(process.cwd(), "database_config.json");
      if (import_fs2.default.existsSync(configPath)) {
        const fileContent = import_fs2.default.readFileSync(configPath, "utf8");
  ...
  const ensureMySQLSchema = async (connection) => {
  ...
  const getMySQLConnection = async () => {
    try {
      if (pool) {
        ...
        if (!connection2.__isIntercepted) {
          const originalEnd = connection2.end?.bind(connection2);
          connection2.end = async () => {
            try {
              connection2.release();
            } catch (err) {
          ...
          connection2.__isIntercepted = true;
        }
        return connection2;
      }
      ...
      if (!connection.__isIntercepted) {
        ...
        connection.__isIntercepted = true;
      }
      return connection;
    } catch (err) {
      console.warn("--------------------------------------------------------------------------------");
  ...
  const findAndSyncUser = async (decodedToken) => {
    let userData = null;
    let conn = null;
    ...
    if (!userData && email) {
      try {
        conn = await getMySQLConnection();
        const [emailRows] = await conn.query("SELECT * FROM users WHERE LOWER(email) = ?", [email.toLowerCase()]);
        if (emailRows && emailRows.length > 0) {
          ...
          const [updatedRows] = await conn.query("SELECT * FROM users WHERE uid = ?", [uid]);
          if (updatedRows && updatedRows.length > 0) {
            userData = updatedRows[0];
          } else {
            userData = { ...tempUserRow, uid };
          }
        }
      } catch (mysqlEmailErr) {
    ...
    if (!userData) {
      ...
      userData = {
        ...
        pickupUpdatedAt: role === "student" ? (/* @__PURE__ */ new Date()).toISOString() : "",
        dropoffStatus: role === "student" ? "pending" : "",
        dropoffUpdatedAt: role === "student" ? (/* @__PURE__ */ new Date()).toISOString() : "",
        pickedAt: "",
  ...
  const verifyTokenResilient = async (token) => {
    try {
      if (auth) {
  ...
  const verifyAdmin = async (req, res, next) => {
    ...
    try {
      ...
      req.user = { ...decodedToken, ...userData || {}, role, orgId, isSuperAdmin, isOrgAdmin: true };
      next();
    } catch (error) {
      console.error("Token verification failed:", error);
      res.status(401).json({ error: "Invalid token", details: error.message });
  ...
  const verifyAnyUser = async (req, res, next) => {
    ...
    try {
      ...
      const isSuperAdmin = isSuperAdminByEmail || role === "super_admin";
      const isOrgAdmin = role === "org_admin";
      req.user = { ...decodedToken, ...userData || {}, role, orgId, isSuperAdmin, isOrgAdmin };
      next();
    } catch (error) {
      console.error("Token verification failed:", error);
      res.status(401).json({ error: "Invalid token", details: error.message });
  ...
  const handleFirebaseAdminError = (e, res, context) => {
    ...
    const isPermissionDenied = errorStr.includes("permission_denied") || errorStr.includes("insufficient permissions") || e.code === 7 || e.status === 7;
    const isRestricted = isPermissionDenied || errorStr.includes("identitytoolkit.googleapis.com") || errorStr.includes("identity toolkit api") || errorStr.includes("service_disabled") || errorStr.includes("accessnotconfigured") || errorStr.includes("user_project_denied") || errorStr.includes("serviceusage.serviceusageconsumer") || errorStr.includes("firestore") || errorStr.includes("cloud datastore user") || errorStr.includes("iam-admin");
    if (isRestricted) {
    ...
    try {
      ...
      if (orgId) {
        ...
        try {
          ...
          const [rows] = await connOrg.query("SELECT * FROM organizations WHERE id = ?", [orgId]);
          if (rows && rows.length > 0) {
            orgRow = rows[0];
          }
        } catch (mysqlErr) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    try {
      ...
      try {
        conn = await getMySQLConnection();
        const [rows] = await conn.query("SELECT * FROM users WHERE LOWER(email) = ?", [email.toLowerCase()]);
        if (rows && rows.length > 0) {
          userRow = rows[0];
        }
      } catch (mysqlErr) {
    ...
    try {
      ...
      if (firestoreDb) {
        try {
          ...
          orgsList = snapshot.docs.map((doc) => {
            const data = doc.data();
            return {
              id: doc.id,
              name: data.name || "",
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    ...
    try {
      ...
      try {
        conn = await getMySQLConnection();
        const [rows] = await conn.query("SELECT * FROM users WHERE LOWER(email) = ?", [email]);
        if (rows && rows.length > 0) {
          userRow = rows[0];
        }
      } catch (mysqlErr) {
  ...
  app.post("/api/admin/create-client", verifySuperAdmin, async (req, res) => {
    ...
    try {
      ...
      const udoc = {
        ...
        phone: mobile || null,
        // Map mobile to phone in users collection
        role: "org_admin",
        orgId: orgId || null,
        forcePasswordChange: true,
        updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  ...
  app.post("/api/admin/create-org-user", verifyAdmin, async (req, res) => {
    try {
      ...
      if (studentId) {
        ...
        try {
          const existingId = await firestoreDb.collection("users").where("orgId", "==", orgId).where("studentId", "==", studentId).limit(1).get();
          if (!existingId.empty) {
            isDuplicate = true;
          }
        } catch (fsErr) {
          ...
          try {
            conn = await getMySQLConnection();
            const [rows] = await conn.query("SELECT uid FROM users WHERE orgId = ? AND studentId = ?", [orgId, studentId]);
            if (rows && rows.length > 0) {
              isDuplicate = true;
            }
          } catch (mysqlErr) {
  ...
  app.post("/api/admin/resend-creds", verifyAdmin, async (req, res) => {
    try {
      ...
      try {
        ...
        const [rows] = await connCreds.query("SELECT * FROM users WHERE uid = ?", [uid]);
        if (rows && rows.length > 0) {
          userData = rows[0];
        }
      } catch (mysqlErr) {
              ...
              if (existingAuthUser.uid !== uid) {
                ...
                try {
                  firestoreDb.collection("users").doc(existingAuthUser.uid).set({
                    ...
                    forcePasswordChange: true,
                    updatedAt: import_firestore.FieldValue.serverTimestamp()
                  }, { merge: true }).catch((err) => console.warn("Background set for mig failed:", err.message));
                } catch (pe) {
                }
              }
            } else {
  ...
  app.get("/api/records/user-data", async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader?.startsWith("Bearer ")) return res.status(401).json({ error: "Unauthorized" });
    const token = authHeader.split("Bearer ")[1];
    let conn = null;
    try {
      ...
      const uid = decodedToken.uid;
      conn = await getMySQLConnection();
      const [userRows] = await conn.query("SELECT * FROM users WHERE uid = ?", [uid]);
      ...
      const org = orgRaw ? {
        ...
        location: orgRaw.latitude !== null && orgRaw.longitude !== null && orgRaw.latitude !== void 0 && orgRaw.longitude !== void 0 ? { lat: Number(orgRaw.latitude), lng: Number(orgRaw.longitude) } : null
      } : null;
      let routeRows = [];
      if (routeId) {
        const [rows] = await conn.query("SELECT * FROM routes WHERE orgId = ? AND id = ?", [orgId, routeId]);
        routeRows = rows;
      } else {
        const [rows] = await conn.query("SELECT * FROM routes WHERE orgId = ?", [orgId]);
        routeRows = rows;
      }
      let orgUsersRows = [];
      if (routeId) {
      ...
      const mappedUsers = orgUsersRows.map((u) => {
        let notificationsArr = [];
        try {
      ...
      const routes = routeRows.map((r) => {
        ...
        const assignedDriver = mappedUsers.find((u) => u.role === "driver" && u.routeId === r.id);
        return {
          ...r,
      ...
      const trips = tripRows.map((t) => {
        const locObj = t.currentLat !== null && t.currentLng !== null && t.currentLat !== void 0 && t.currentLng !== void 0 ? { lat: Number(t.currentLat), lng: Number(t.currentLng) } : null;
        let tManifest = [];
        try {
      ...
      const [vehicleRows] = await conn.query("SELECT * FROM vehicles WHERE orgId = ?", [orgId]);
      const vehicles = vehicleRows.map((v) => {
        const locObj = v.latitude !== null && v.longitude !== null && v.latitude !== void 0 && v.longitude !== void 0 ? { lat: Number(v.latitude), lng: Number(v.longitude) } : null;
        return {
          ...v,
      ...
      res.json({
        ...
        org,
        vehicles,
        drivers: mappedUsers.filter((u) => u.role === "driver"),
        members: mappedUsers.filter((u) => u.role === "user"),
        users: mappedUsers,
        routes,
        trips,
        payments: [],
  ...
  app.get("/api/records/admin-data", verifyAdmin, async (req, res) => {
    let conn = null;
    try {
      conn = await getMySQLConnection();
      ...
      if (isSuperAdmin) {
        ...
        const mappedOrgs = orgs.map((o) => {
          ...
          return {
            ...
            location: o.latitude !== null && o.longitude !== null && o.latitude !== void 0 && o.longitude !== void 0 ? { lat: Number(o.latitude), lng: Number(o.longitude) } : null,
            billing: {
              price,
              gstPercent,
              gstAmount,
        ...
        res.json({
          ...
          users: mappedUsers,
          settings: { sectors: ["Education", "Corporate", "Logistics", "Retail"] }
        ...
        const [vehicles] = await conn.query("SELECT * FROM vehicles WHERE orgId = ?", [orgId]);
        const [users] = await conn.query("SELECT * FROM users WHERE orgId = ?", [orgId]);
        const [routes] = await conn.query("SELECT * FROM routes WHERE orgId = ?", [orgId]);
        const [trips] = await conn.query("SELECT * FROM trips WHERE orgId = ?", [orgId]);
        const [payments] = await conn.query("SELECT * FROM payments WHERE orgId = ?", [orgId]);
        const [logs] = await conn.query("SELECT * FROM logs WHERE organization = ? OR entity = ?", [orgId, orgId]);
        let classes = [];
        try {
          const [classRows] = await conn.query("SELECT * FROM classes WHERE orgId = ?", [orgId]);
        ...
        const mappedOrg = org ? {
          ...
          location: org.latitude !== null && org.longitude !== null && org.latitude !== void 0 && org.longitude !== void 0 ? { lat: Number(org.latitude), lng: Number(org.longitude) } : null
        } : null;
        const mappedUsers = users.map((u) => {
          let notificationsArr = [];
          try {
        ...
        const drivers = mappedUsers.filter((u) => u.role === "driver");
        const members = mappedUsers.filter((u) => u.role === "user");
        const mappedLogs = logs.map((l) => ({
          ...l,
          details: l.description || l.details || "",
          userEmail: l.operator || l.userEmail || ""
        ...
        res.json({
          ...
          members,
          users,
          // expose all users for route management etc.
          routes: mappedRoutes,
          trips: mappedTrips,
          payments,
          classes,
  ...
  app.post("/api/records/save", verifyAnyUser, async (req, res) => {
    ...
    try {
      ...
      if (operation === "delete") {
        ...
        try {
          if (table === "users") {
            firestoreDb.collection("users").doc(id).delete().catch((err) => console.error("Firestore sync delete user error:", err.message));
            firestoreDb.collection("trips").where("driverId", "==", id).get().then((snap) => {
              const batch = firestoreDb.batch();
              snap.docs.forEach((doc) => batch.delete(doc.ref));
              return batch.commit();
            }).catch((err) => console.error("Firestore sync driver cleanup error:", err.message));
          } else if (table === "routes") {
            firestoreDb.collection("routes").doc(id).delete().catch((err) => console.error("Firestore sync delete route error:", err.message));
            firestoreDb.collection("users").where("routeId", "==", id).get().then((snap) => {
              const batch = firestoreDb.batch();
              snap.docs.forEach((doc) => batch.update(doc.ref, { routeId: "", pickupPointId: "" }));
              return batch.commit();
            }).catch((err) => console.error("Firestore sync route user-unassign error:", err.message));
            firestoreDb.collection("trips").where("routeId", "==", id).get().then((snap) => {
              const batch = firestoreDb.batch();
              snap.docs.forEach((doc) => batch.delete(doc.ref));
              return batch.commit();
            }).catch((err) => console.error("Firestore sync route trips cleanup error:", err.message));
          } else if (table === "vehicles") {
            firestoreDb.collection("vehicles").doc(id).delete().catch((err) => console.error("Firestore sync delete vehicle error:", err.message));
            firestoreDb.collection("users").where("vehicleId", "==", id).get().then((snap) => {
              const batch = firestoreDb.batch();
              snap.docs.forEach((doc) => batch.update(doc.ref, { vehicleId: "" }));
              return batch.commit();
            }).catch((err) => console.error("Firestore sync vehicle user-unassign error:", err.message));
            firestoreDb.collection("trips").where("vehicleId", "==", id).get().then((snap) => {
              const batch = firestoreDb.batch();
              snap.docs.forEach((doc) => batch.delete(doc.ref));
              return batch.commit();
            }).catch((err) => console.error("Firestore sync vehicle trips cleanup error:", err.message));
          } else {
            firestoreDb.collection(table).doc(id).delete().catch((err) => console.error("Firestore sync delete error:", err.message));
          }
        } catch (fE) {
        }
      ...
      if (table === "organizations") {
        ...
        const orgLat = mergedData.latitude !== void 0 ? mergedData.latitude : mergedData.location?.lat !== void 0 ? mergedData.location.lat : null;
        const orgLng = mergedData.longitude !== void 0 ? mergedData.longitude : mergedData.location?.lng !== void 0 ? mergedData.location.lng : null;
        ...
        const uDate = mergedData.updatedAt || (/* @__PURE__ */ new Date()).toISOString();
        ...
        const uStatus = mergedData.status !== void 0 ? mergedData.status : null;
        const uStatusUpdatedAt = mergedData.statusUpdatedAt !== void 0 ? mergedData.statusUpdatedAt : null;
        const uPickupStatus = mergedData.pickupStatus !== void 0 ? mergedData.pickupStatus : null;
        const uPickupUpdatedAt = mergedData.pickupUpdatedAt !== void 0 ? mergedData.pickupUpdatedAt : null;
        const uDropoffStatus = mergedData.dropoffStatus !== void 0 ? mergedData.dropoffStatus : null;
        const uDropoffUpdatedAt = mergedData.dropoffUpdatedAt !== void 0 ? mergedData.dropoffUpdatedAt : null;
        const uPickedAt = mergedData.pickedAt !== void 0 ? mergedData.pickedAt : null;
        await conn.query(
          ...
          [
            ...
            uOrgId,
            mergedData.routeId || "",
            mergedData.vehicleId || "",
            mergedData.pickupPointId || "",
        ...
        const busLat = mergedData.latitude !== void 0 ? mergedData.latitude : mergedData.location?.lat !== void 0 ? mergedData.location.lat : null;
        const busLng = mergedData.longitude !== void 0 ? mergedData.longitude : mergedData.location?.lng !== void 0 ? mergedData.location.lng : null;
        ...
        const tLat = tripData.currentLat !== void 0 ? tripData.currentLat : tripData.location?.lat !== void 0 ? tripData.location.lat : null;
        const tLng = tripData.currentLng !== void 0 ? tripData.currentLng : tripData.location?.lng !== void 0 ? tripData.location.lng : null;
        ...
        const lTime = data.timestamp || (/* @__PURE__ */ new Date()).toISOString();
        await conn.query(
        ...
        const pTime = data.timestamp || (/* @__PURE__ */ new Date()).toISOString();
        await conn.query(
      ...
      try {
        if (table === "users") {
          firestoreDb.collection("users").doc(id).set(mergedData, { merge: true }).catch((err) => console.error("Firestore sync set error:", err.message));
        } else {
          firestoreDb.collection(table).doc(id).set(mergedData, { merge: true }).catch((err) => console.error("Firestore sync set error:", err.message));
        }
      } catch (fE) {
      }
  ...
  app.post("/api/records/save-batch", verifyAnyUser, async (req, res) => {
  ...
  const getProviderName = (provider) => {
    switch (provider) {
      case "bluehost":
        return "Bluehost";
      case "hostinger_shared":
        return "Hostinger Shared";
      case "hostinger_vps":
        return "Hostinger VPS";
      case "aws":
        return "AWS RDS";
      case "digitalocean":
        return "DigitalOcean";
  ...
  async function syncFirestoreChangeToMySQL(collectionName, docId, customData) {
    let connection = null;
    try {
      const config = await getMySQLConfig();
      if (!config.host || !config.user || !config.database) {
      ...
      if (!d) {
        if (collectionName === "payments") {
          ...
          const targetDoc = paymentsSnap.docs.find((doc) => doc.id === docId);
          if (targetDoc) {
            d = targetDoc.data();
      ...
      if (collectionName === "organizations") {
        ...
        const onboard = d.onboardDate?.seconds ? new Date(d.onboardDate.seconds * 1e3).toISOString() : d.onboardDate ? String(d.onboardDate) : "";
        const expiry = d.expiryDate?.seconds ? new Date(d.expiryDate.seconds * 1e3).toISOString() : d.expiryDate ? String(d.expiryDate) : "";
        let lat = null;
        let lng = null;
        if (d.location) {
          ...
          lat = d.latitude !== void 0 ? Number(d.latitude) : null;
          lng = d.longitude !== void 0 ? Number(d.longitude) : null;
        }
        ...
        const uDate = d.updatedAt?.seconds ? new Date(d.updatedAt.seconds * 1e3).toISOString() : d.updatedAt ? String(d.updatedAt) : "";
        ...
        const sTime = d.startTime?.seconds ? new Date(d.startTime.seconds * 1e3).toISOString() : d.startTime ? String(d.startTime) : "";
        const eTime = d.endTime?.seconds ? new Date(d.endTime.seconds * 1e3).toISOString() : d.endTime ? String(d.endTime) : "";
        let lat = null;
        let lng = null;
        if (d.location) {
          lat = Number(d.location.lat ?? d.location.latitude ?? null);
          ...
          lat = d.currentLat !== void 0 ? Number(d.currentLat) : null;
          lng = d.currentLng !== void 0 ? Number(d.currentLng) : null;
        }
        if (isNaN(lat)) lat = null;
        if (isNaN(lng)) lng = null;
        ...
        const lTime = d.timestamp?.seconds ? new Date(d.timestamp.seconds * 1e3).toISOString() : d.timestamp ? String(d.timestamp) : "";
        await connection.query(
          `INSERT INTO logs (id, entity, action, description, organization, operator, timestamp) 
           VALUES (?, ?, ?, ?, ?, ?, ?) 
           ON DUPLICATE KEY UPDATE entity=?, action=?, description=?, organization=?, operator=?, timestamp=?`,
        ...
        const pTime = d.timestamp?.seconds ? new Date(d.timestamp.seconds * 1e3).toISOString() : d.timestamp ? String(d.timestamp) : "";
        await connection.query(
          `INSERT INTO payments (id, orgId, amount, paymentMode, transactionId, note, timestamp) 
           VALUES (?, ?, ?, ?, ?, ?, ?) 
           ON DUPLICATE KEY UPDATE orgId=?, amount=?, paymentMode=?, transactionId=?, note=?, timestamp=?`,
  ...
  function registerCollectionSync(collectionName) {
    let isInitial = true;
  ...
  async function syncMySQLToFirestore() {
    let connection = null;
    try {
      const config = await getMySQLConfig();
      if (!config.host || !config.user || !config.database) {
      ...
      try {
        if (tableNames.includes("routes")) {
          ...
          for (const r of routes) {
            ...
            try {
              if (r.startPoint) startPoint = JSON.parse(r.startPoint);
            } catch (e) {
            }
            try {
              if (r.endPoint) endPoint = JSON.parse(r.endPoint);
            } catch (e) {
            }
            try {
              if (r.stops) stops = typeof r.stops === "string" ? JSON.parse(r.stops) : r.stops;
            } catch (e) {
            }
            try {
              if (r.pickupPoints) pickupPoints = typeof r.pickupPoints === "string" ? JSON.parse(r.pickupPoints) : r.pickupPoints;
            } catch (e) {
            }
            const healPoint = (pt) => {
              ...
              if (ptLat >= 12 && ptLat <= 14.1 && ptLng >= 79 && ptLng <= 81.1) {
                const nameLower = (pt.name || "").toLowerCase();
                let newLat = 17.4504 + (ptLat - 13.0400731);
                let newLng = 78.3808 + (ptLng - 80.2159247);
                if (nameLower.includes("ashok nagar")) {
                  newLat = 17.5029;
                  newLng = 78.3015;
                } else if (nameLower.includes("bhel")) {
                  newLat = 17.5029;
                  newLng = 78.2934;
                } else if (nameLower.includes("masjid banda")) {
                  newLat = 17.45;
                  newLng = 78.358;
                } else if (nameLower.includes("patancheru")) {
                  newLat = 17.5226;
                  newLng = 78.2612;
                }
                return {
            ...
            if (Array.isArray(stops)) {
              stops = stops.map((s) => {
                if (s && s.lat !== void 0) {
                  ...
                  if (sLat >= 12 && sLat <= 14.1 && sLng >= 79 && sLng <= 81.1) {
            ...
            if (Array.isArray(pickupPoints)) {
              pickupPoints = pickupPoints.map((p) => {
                if (p && p.lat !== void 0) {
                  ...
                  if (pLat >= 12 && pLat <= 14.1 && pLng >= 79 && pLng <= 81.1) {
      ...
      const [mysqlOrgs] = await connection.query("SELECT * FROM organizations");
      const mysqlOrgIds = new Set(mysqlOrgs.map((o) => String(o.id)));
      const orgDocs = await firestoreDb.collection("organizations").get();
      const orgMap = new Map(orgDocs.docs.map((doc) => [doc.id, doc.data()]));
      ...
      for (const mo of mysqlOrgs) {
        const newData = {
          ...
          latitude: mo.latitude !== null && mo.latitude !== void 0 ? Number(mo.latitude) : null,
          longitude: mo.longitude !== null && mo.longitude !== void 0 ? Number(mo.longitude) : null,
          location: mo.latitude !== null && mo.latitude !== void 0 && mo.longitude !== null && mo.longitude !== void 0 ? { lat: Number(mo.latitude), lng: Number(mo.longitude) } : null,
          eduType: mo.eduType || ""
        };
        const existing = orgMap.get(String(mo.id));
        if (existing) {
          const isSame = (existing.name || "") === newData.name && (existing.sector || "") === newData.sector && (existing.mobile || "") === newData.mobile && (existing.email || "") === newData.email && (existing.logoUrl || "") === newData.logoUrl && (existing.address || "") === newData.address && (existing.subscriptionPlan || "basic") === newData.subscriptionPlan && (existing.status || "active") === newData.status && Number(existing.totalPaidAmount || 0) === newData.totalPaidAmount && Number(existing.billing?.price || 0) === newData.billing.price && String(existing.onboardDate || "") === newData.onboardDate && String(existing.expiryDate || "") === newData.expiryDate && existing.latitude === newData.latitude && existing.longitude === newData.longitude && existing.location?.lat === newData.location?.lat && existing.location?.lng === newData.location?.lng && (existing.eduType || "") === newData.eduType;
          if (isSame) continue;
        }
        await firestoreDb.collection("organizations").doc(String(mo.id)).set(newData, { merge: true });
        upsertCount++;
      }
      const [mysqlUsers] = await connection.query("SELECT * FROM users");
      const mysqlUserUids = new Set(mysqlUsers.map((u) => String(u.uid)));
      ...
      for (const mu of mysqlUsers) {
        ...
        if (existing) {
          const isSame = (existing.email || "") === newData.email && (existing.name || "") === newData.name && (existing.phone || "") === newData.phone && (existing.role || "user") === newData.role && (existing.orgId || "") === newData.orgId && (existing.routeId || "") === newData.routeId && (existing.vehicleId || "") === newData.vehicleId && (existing.pickupPointId || "") === newData.pickupPointId && (existing.studentId || "") === newData.studentId && (existing.classId || "") === newData.classId && (existing.section || "") === newData.section && (existing.avatarUrl || null) === newData.avatarUrl && JSON.stringify(existing.notifications || []) === JSON.stringify(newData.notifications);
          if (isSame) continue;
        }
      ...
      if (tableNames.includes("vehicles")) {
        const [mysqlVehicles] = await connection.query("SELECT * FROM vehicles");
        const mysqlVehicleIds = new Set(mysqlVehicles.map((v) => String(v.id)));
        const vehicleDocs = await firestoreDb.collection("vehicles").get();
        const vehicleMap = new Map(vehicleDocs.docs.map((doc) => [doc.id, doc.data()]));
        ...
        for (const mv of mysqlVehicles) {
          ...
          if (existing) {
            const isSame = (existing.orgId || "") === newData.orgId && (existing.name || "") === newData.name && (existing.number || "") === newData.number && (existing.type || "") === newData.type && Number(existing.capacity || 0) === newData.capacity && (existing.status || "active") === newData.status;
            if (isSame) continue;
          }
      ...
      if (tableNames.includes("routes")) {
        const [mysqlRoutes] = await connection.query("SELECT * FROM routes");
        const mysqlRouteIds = new Set(mysqlRoutes.map((r) => String(r.id)));
        const routeDocs = await firestoreDb.collection("routes").get();
        const routeMap = new Map(routeDocs.docs.map((doc) => [doc.id, doc.data()]));
        ...
        for (const mr of mysqlRoutes) {
          ...
          if (existing) {
            const isSame = (existing.orgId || "") === newData.orgId && (existing.name || "") === newData.name && JSON.stringify(existing.startPoint) === JSON.stringify(newData.startPoint) && JSON.stringify(existing.endPoint) === JSON.stringify(newData.endPoint) && (existing.distance || "") === newData.distance && JSON.stringify(existing.stops || []) === JSON.stringify(newData.stops) && JSON.stringify(existing.pickupPoints || []) === JSON.stringify(newData.pickupPoints);
            if (isSame) continue;
          }
      ...
      if (tableNames.includes("trips")) {
        const [mysqlTrips] = await connection.query("SELECT * FROM trips");
        const mysqlTripIds = new Set(mysqlTrips.map((t) => String(t.id)));
        const tripDocs = await firestoreDb.collection("trips").get();
        const tripMap = new Map(tripDocs.docs.map((doc) => [doc.id, doc.data()]));
        ...
        for (const mt of mysqlTrips) {
          ...
          const newData = {
            ...
            currentLat: mt.currentLat !== null && mt.currentLat !== void 0 ? Number(mt.currentLat) : null,
            currentLng: mt.currentLng !== null && mt.currentLng !== void 0 ? Number(mt.currentLng) : null,
          ...
          if (existing) {
            const isSame = (existing.orgId || "") === newData.orgId && (existing.driverId || "") === newData.driverId && (existing.vehicleId || "") === newData.vehicleId && (existing.routeId || "") === newData.routeId && (existing.status || "live") === newData.status && (existing.startAddress || "") === newData.startAddress && (existing.endAddress || "") === newData.endAddress && String(existing.startTime || "") === String(newData.startTime) && String(existing.endTime || "") === String(newData.endTime) && String(existing.startedAt || "") === String(newData.startedAt) && String(existing.endedAt || "") === String(newData.endedAt) && existing.currentLat === newData.currentLat && existing.currentLng === newData.currentLng && (existing.currentStopId || "") === newData.currentStopId && (existing.eta || "") === newData.eta && JSON.stringify(existing.manifest || []) === JSON.stringify(newData.manifest);
            if (isSame) continue;
          }
      ...
      if (tableNames.includes("payments")) {
        const [mysqlPayments] = await connection.query("SELECT * FROM payments");
        const mysqlPaymentIds = new Set(mysqlPayments.map((p) => String(p.id)));
        const paymentsSnap = await firestoreDb.collectionGroup("payments").get();
        const paymentsMap = new Map(paymentsSnap.docs.map((doc) => [doc.id, doc.data()]));
        ...
        for (const mp of mysqlPayments) {
          ...
          if (existing) {
            const isSame = Number(existing.amount || 0) === newData.amount && (existing.paymentMode || "") === newData.paymentMode && (existing.transactionId || "") === newData.transactionId && (existing.note || "") === newData.note && String(existing.timestamp || "") === String(newData.timestamp);
            if (isSame) continue;
          }
      ...
      if (tableNames.includes("classes")) {
        const [mysqlClasses] = await connection.query("SELECT * FROM classes");
        const mysqlClassIds = new Set(mysqlClasses.map((c) => String(c.id)));
        const classDocs = await firestoreDb.collection("classes").get();
        const classMap = new Map(classDocs.docs.map((doc) => [doc.id, doc.data()]));
        ...
        for (const mc of mysqlClasses) {
          ...
          if (existing) {
            const isSame = (existing.orgId || "") === newData.orgId && (existing.name || "") === newData.name && JSON.stringify(existing.sections || []) === JSON.stringify(newData.sections);
            if (isSame) continue;
          }
  ...
  app.post("/api/admin/db-save", verifySuperAdmin, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        let existingConfig = {};
        if (import_fs2.default.existsSync(configPath)) {
          existingConfig = JSON.parse(import_fs2.default.readFileSync(configPath, "utf8"));
        }
        const mergedConfig = { ...existingConfig, ...updateData };
        import_fs2.default.writeFileSync(configPath, JSON.stringify(mergedConfig, null, 2), "utf8");
  ...
  app.post("/api/admin/db-test", verifySuperAdmin, async (req, res) => {
    ...
    try {
      ...
      const testConfig = {
        host: host !== void 0 ? host : configObj.host,
        user: user !== void 0 ? user : configObj.user,
        password: password !== void 0 ? password : configObj.password,
        database: database !== void 0 ? database : configObj.database,
        port: port !== void 0 ? parseInt(port) : configObj.port,
        provider: provider !== void 0 ? provider : configObj.provider
      };
  ...
  app.post("/api/admin/db-sync", verifySuperAdmin, async (req, res) => {
    ...
    try {
      ...
      const parseNumberValue = (v) => {
        if (v === null || v === void 0 || isNaN(Number(v))) return null;
        return Number(v);
      };
        ...
        for (const pDoc of paymentsSnap.docs) {
          ...
          await connection.query(
            ...
            [
              ...
              pd.transactionId || "",
              pd.note || "",
              pd.timestamp?.seconds ? new Date(pd.timestamp.seconds * 1e3).toISOString() : pd.timestamp ? String(pd.timestamp) : ""
            ]
          );
          stats.payments++;
        }
      ...
      for (const dDoc of logsSnap.docs) {
        ...
        await connection.query(
          ...
          [
            ...
            d.organization || "",
            d.operator || "",
            d.timestamp?.seconds ? new Date(d.timestamp.seconds * 1e3).toISOString() : d.timestamp ? String(d.timestamp) : ""
          ]
        );
        stats.logs++;
      }
  ...
  async function generateSqlDumpContent() {
    if (!firestoreDb) {
...
`;
    sqlDump += `-- Export Date: ${(/* @__PURE__ */ new Date()).toISOString()}
`;
    sqlDump += `-- --------------------------------------------------------
...
`;
        ...
        if (paymentsSnap.size > 0) {
          for (const pDoc of paymentsSnap.docs) {
            ...
            const pTime = pd.timestamp?.seconds ? new Date(pd.timestamp.seconds * 1e3).toISOString() : pd.timestamp ? String(pd.timestamp) : "";
...
`;
  ...
  async function performDailyBackupCheckAndMaintenance(explicitFilename) {
    ...
    const targetFilePath = import_path2.default.join(backupsDir, targetFilename);
    import_fs2.default.writeFileSync(targetFilePath, dumpData, "utf8");
    console.log(`[Backup Engine] Database backup successfully persisted: ${targetFilePath}`);
    try {
      const files = import_fs2.default.readdirSync(backupsDir);
      const backupFiles = files.filter((f) => f.startsWith("backup_") && f.endsWith(".sql")).map((f) => {
        const stats = import_fs2.default.statSync(import_path2.default.join(backupsDir, f));
      ...
      while (backupFiles.length > 10) {
        ...
        if (oldestFile) {
          const fileDeletePath = import_path2.default.join(backupsDir, oldestFile.filename);
          import_fs2.default.unlinkSync(fileDeletePath);
  ...
  try {
    const initialDir = import_path2.default.join(process.cwd(), "backups");
    if (!import_fs2.default.existsSync(initialDir)) {
      import_fs2.default.mkdirSync(initialDir, { recursive: true });
    }
  } catch (err) {
  ...
  setInterval(async () => {
    try {
      ...
      if (now.getHours() === 2) {
        ...
        const localPath = import_path2.default.join(process.cwd(), "backups", autoFilename);
  ...
  app.get("/api/admin/backups-list", verifySuperAdmin, async (req, res) => {
    try {
      ...
      const files = import_fs2.default.readdirSync(backupsDir);
      const listData = files.filter((f) => f.startsWith("backup_") && f.endsWith(".sql")).map((f) => {
        const fullPath = import_path2.default.join(backupsDir, f);
  ...
  app.get("/api/admin/backups-download/:filename", verifySuperAdmin, async (req, res) => {
    try {
      ...
      const cleanFilename = import_path2.default.basename(filename);
      const absoluteBackupPath = import_path2.default.join(process.cwd(), "backups", cleanFilename);
      if (!import_fs2.default.existsSync(absoluteBackupPath) || !cleanFilename.startsWith("backup_") || !cleanFilename.endsWith(".sql")) {
  ...
  app.delete("/api/admin/backups/:filename", verifySuperAdmin, async (req, res) => {
    try {
      const { filename } = req.params;
      const cleanFilename = import_path2.default.basename(filename);
      const targetFilePath = import_path2.default.join(process.cwd(), "backups", cleanFilename);
      if (!import_fs2.default.existsSync(targetFilePath) || !cleanFilename.startsWith("backup_") || !cleanFilename.endsWith(".sql")) {
        return res.status(404).json({ error: "Selected relational database snapshot not found." });
      }
      import_fs2.default.unlinkSync(targetFilePath);
